...
public class SimpleThreadpool {
    ...
    /**
     * Получает новый экземпляр пула потоков с количеством потоков, равным количеству доступных процессоров (или потоков ЦП)
     *
    ...
    /**
     * У нас есть два способа остановить Threadpool - terminate() или stop().
     *
     * Очищает очередь исполняемых файлов и останавливает пул потоков. Все
     * выполняемые в данный момент исполняемые файлы продолжат выполнение.
     *
     * Метод terminate() является более резким, чем метод stop(), так как он
     * очищает очередь, а затем вызывает stop(). Если есть какие-либо
     ...
     * очереди.
     *
     * Метод stop() устанавливает execute флаг в false, и Threadpool
