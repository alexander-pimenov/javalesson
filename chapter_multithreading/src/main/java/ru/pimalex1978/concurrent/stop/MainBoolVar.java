 ...
 *
 */
public class MainBoolVar {
    /*переменная для остановки потока isStopped это не лучший вариант,
     * т.к. не смотря на то что в main методе через 3 секунды она
     * будет изменена на true, а при этом задач будет мало сгенерированно,
     * например всего 2, то т.к. очередь опустеет после 2 задач и
     * поток заблокируется на методе wait() и не сможет в цикле проверить
     * условие. Он будет висеть. Программа завснет. Не смотря на то, что
     * метод main пойдет дальше и закончит свое выполнение.
     * */
    ...
    public static void main(String[] args) {
         ...
         * Измени количество циклов на 2 и увидишь, что программа не
         * сможет завершиться.*/
        for (int i = 0; i < 5; i++) {
            queue.put(getTask());
        }
        ...
        /*В этом примере isStopped поменяет значение на true через 3 сек и
         * произойдет остановка программы.
         * */
        isStopped = true;
    }
