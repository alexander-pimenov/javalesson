 ...
 * Напомню, что просто убить поток нам нельзя, т.к. это может произойти в середине
 * задачи. Чтобы избежать неприятных ситуаций мы сообщаем потоку, что ему
 * нужно остановиться, а поток уже сам решит, когда он может это сделать.
 * Он довелет свои дела до логического конца и безопасно остановится.
 *
 ...
 * сбрасывается. Чтобы флаг остановки стал остался true и что бы мы отследили
 * факт остановки то нужно в месте отлова InterruptedException (в блоке
 * catch) вызвать interrupt() еще раз. Поток сам у себя еще раз перевызывает
 * isInterrupt() для того чтобы установить значение флага в true, чтобы код
 * выше мог отследить что остановка была.
 *
 ...
 * клиентский код, т.е. код к-рый использует эту очередь. В данном случае
 ...
 * обработать Exception в коде, который использует очередь. И это более
 * правильно, потому что мы предоставляем пользователю очереди самому
 * решать что делать с этим Exception: или мы его учтем, или отложим,
 * или игнорируем.
 * В нашем случае мы в текущем потоке перевызовем interrupt(). И это
 * более граммотно, т.к. мы не проглатываем InterruptedException, а
 * обрабатываем его так следует это делать.
 *
 ...
 * ни к чему не приведут. Проглатывая InterruptedException мы лишаем потоки
 * возможности завершиться по нормальному.
 *
 */
public class MainInterrupt {
    ...
    public static void main(String[] args) {
         ...
         * до worker.interrupt(); - это правильное завершение работы потоков.
         * */
        for (int i = 0; i < 2; i++) {
            queue.put(getTask());
        }
