 ...
 * поток вырывется вперед и завладевает этим методом, то другие
 * потоки будут ждать пока не будет вызван метод .unlock()
 * <p>
 * Если на одном потоке мы нескольок раз вызовем метод lock(),
...
class Task {
     ...
     * Код будет потокобезопастным, т.е. два потока смогут
     * писать в одну переменную в одим и тот же момент времени.
