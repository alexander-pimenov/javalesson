package ru.pimalex1978.stream.suleimanov.util;
...
public class StreamAPIUtil {
    ...
    //Вернет первого кто есть в коллекции с max значением.
    // max() возвращает Optional поэтому ставим в конце orElse(null), который говорит что
    //если мы max не найдем, т.е. пустая коллекция, то вернется null.
    ...
    //Вернет первого кто есть в коллекции с min значением.
    // min() возвращает Optional поэтому ставим в конце orElse(null), который говорит что
    //если мы min не найдем, т.е. пустая коллекция, то вернется null.
