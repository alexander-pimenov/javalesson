...
import ru.pimalex1978.a11exe.WrongMoveException;
...
public class TicTacToe3T implements TicTacToe {
    ...
    private final String wrongSizeMsg = "Введите правильные кординаты. Размер поля %d на %d";
    private final String fieldBusyMsg = "Введите правильные координаты. Ячейка (%d %d) уже занята";
    ...
    public Mark getMark(Point point) throws WrongMoveException {
        if (point.getX() >= table.length || point.getY() >= table.length) {
            throw new WrongMoveException(String.format(wrongSizeMsg, table.length, table.length));
        }
